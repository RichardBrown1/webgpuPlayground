...
export const Cube = async (gpu: GPU, device: GPUDevice) => {
    ...
    //var camera = createCamera(gpu.getPreferredCanvasFormat, vp.cameraOption);
    ...
    let textureView = canvas.context.getCurrentTexture().createView();
    ...
    function draw() {
        ...
        textureView = canvas.context.getCurrentTexture().createView();
        let colorAttachmentArr = Array.from(renderPassDescription.colorAttachments);
        if(colorAttachmentArr[0] !== null) {
            colorAttachmentArr[0].view = textureView;
        }
        renderPassDescription.colorAttachments = colorAttachmentArr;
    ...
    function CreateAnimation (draw:any, rotation:vec3 = vec3.fromValues(0,0,0), isAnimation = true ) {
        function frame() {
            rotation[0] += 0.1;
            rotation[1] += 0.1;
            rotation[2] += 0.1;
